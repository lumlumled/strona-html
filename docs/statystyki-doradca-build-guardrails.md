...
> Cel: panel `/statystyki` z fasadą `/api/stats/*` (backend: Supabase + Zernio), a nad nim AI-doradca w hubie — czat, który widzi całą firmę i odpowiada od razu. Ten dokument to twarde zasady, żeby build nie wpadł w miny w danych i żeby doradca był SZYBKI. Pochodzi z analizy realnej bazy LumLum (lip 2026).
...
- **Close rate = KOHORTOWY.** „Z wycen wysłanych w okresie X — jaki % stał się zamówieniem" (śledzenie kohorty w czasie). NIE „zamknięte_w_tym_miesiącu / wszystkie_w_tym_miesiącu" — wyceny domykają się między miesiącami, naiwna wersja kłamie. To metryka, o którą doradca będzie pytany najczęściej.
...
## 5. AI-doradca — jak go spiąć

- **Mózg = prompt z `docs/fable-doradca-lumlum.md`** (charakter, strategia, cele, zasady). To system prompt.
- **Kontekst = snapshot G** wstrzykiwany co turę (sekcja 4).
- **Narzędzie = jedno**: `stats(group, params)` → `/api/stats/*`, read-only, limit wyniku. Model sam decyduje, kiedy sięgnąć głębiej.
- **Streaming (SSE)** — token po tokenie, to daje uczucie „od razu".
- **Szybki model** do czatu (Fable/Haiku/Sonnet); Opus tylko na głęboką analizę na żądanie.
- **Pamięć** (`doradca_memory`: ustalenia, obietnice) i **proaktywność** (cron „plan na dziś" → push) = DOKŁADKA na potem, nie do v0.
